"""Static contracts for current-head v27 PostgreSQL operations assets."""
...
SCRIPT = ROOT / "deploy/postgres-backup-restore-v27.sh"
FACTS = ROOT / "deploy/postgres-core-facts-v27.sql"
OVERLAY = ROOT / "deploy/postgres-operations-v27.compose.yaml"
V26_FROZEN = {
...
class PostgresOperationsV27Test(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.script = SCRIPT.read_text(encoding="utf-8")
        cls.facts = FACTS.read_text(encoding="utf-8")

    def test_v27_pins_all_six_schema_components(self) -> None:
        ...
        self.assertNotIn("__MATCHING_V3_MANIFEST_SHA256__", self.script)
        subprocess.run(["/bin/sh", "-n", SCRIPT], check=True)

    def test_every_matching_v1_v3_table_has_private_count_and_empty_gate(self) -> None:
    ...
    def test_v27_overlay_only_rebinds_operations_configs(self) -> None:
