#!/usr/bin/env python3
"""Read-only verifier for the v28 PostgreSQL/Matching static release head."""
...
SUCCESS = '{"status":"CURRENT_HEAD_V28_STATIC_VERIFIED"}'
...
MATCHING_MANIFEST_SHA256 = "ff3453c1f86739684dbe255a6ae16a0b5839dacf7ba680b120a50b089aa260e2"
...
HEAD_SQL_SHA256 = {
    ...
    "matching": "726907b4d5f7f0473bc0b826a59134bb59007d34344bb6ddd4ff70a07a477de9",
...
"""
    if overlay != expected_overlay:
        failures.append("postgres-operations-v28-overlay-open")
    if f"EXPECTED_PINS='{HEADS}'" not in script:
        failures.append("postgres-operations-v28-heads-open")
    if f"EXPECTED_CONTRACTS='{EXPECTED_CONTRACTS}'" not in script:
        failures.append("postgres-operations-v28-contracts-open")
    for marker in (
        ...
        if marker not in script:
            failures.append("postgres-operations-v28-matching-contract-open")
            break
    for table in MATCHING_TABLES:
        qualified = f"matching.{table}"
        if script.count(qualified) != 1 or facts.count(qualified) != 1:
            failures.append("postgres-operations-v28-matching-continuity-open")
            break
    for forbidden in (
        "creator_user_id",
        "reviewer_user_id",
        ...
        if forbidden in facts:
            failures.append("postgres-operations-v28-facts-privacy-open")
            break
    return _unique(failures)


def _runbook_failures(value: str) -> tuple[str, ...]:
    begin = "<!-- BEGIN CURRENT_HEAD_V28_CONTRACT -->"
    end = "<!-- END CURRENT_HEAD_V28_CONTRACT -->"
    failures: list[str] = []
    if (
        value.count(begin) != 1
        ...
        failures.append("current-head-v28-markers-open")
    for marker in (
        HEADS,
        IAM_MANIFEST_SHA256,
        IAM_COMBINED_SHA256,
        ...
        MATCHING_MANIFEST_SHA256,
        TAXONOMY_MANIFEST_SHA256,
        "Matching v1-v9 的 27 张 durable domain tables",
        "matching_continuity_counts",
        "STATIC VERIFIED / NOT PRODUCTION EXECUTED",
        '"production_authorized":false',
        "postgres-operations-v28.compose.yaml",
        "v28-iam46-profile5-demand15-trust22-matching9-taxonomy2-drill01",
    ):
        if marker not in value:
            failures.append("current-head-v28-runbook-contract-open")
            break
    return _unique(failures)
...
def verify_repository(root: Path = REPOSITORY_ROOT) -> tuple[str, ...]:
    failures: list[str] = []
    fixture_path = root / "tests/deployment/fixtures/current-head-v28/schema-pins.json"
    fixture_bytes = _bytes(fixture_path)
    fixture = _json(fixture_path)
    if not isinstance(fixture, dict) or fixture_bytes is None:
        failures.append("current-head-v28-fixture-open")
    else:
        canonical = (
        ...
        if fixture_bytes != canonical:
            failures.append("current-head-v28-fixture-canonical-open")
        if (
            ...
            failures.append("current-head-v28-fixture-claim-open")
        if fixture.get("components") != EXPECTED_COMPONENT_PINS:
            failures.append("current-head-v28-fixture-pins-open")

    if "__MATCHING_V4_" in "\n".join(
        (
            MATCHING_MANIFEST_SHA256,
            _read(fixture_path),
            _read(root / "deploy/postgres-backup-restore-v28.sh"),
            _read(root / "docs/operations/current-head-v28.md"),
        )
    ):
        failures.append("current-head-v28-matching-pins-open")

    failures.extend(_manifest_failures(root))
    failures.extend(_historical_prefix_failures(root))
    for relative, expected in {**FROZEN_V26, **FROZEN_V27}.items():
    ...
    failures.extend(
        _operations_failures(
            _read(root / "deploy/postgres-backup-restore-v28.sh"),
            _read(root / "deploy/postgres-core-facts-v28.sql"),
            _read(root / "deploy/postgres-operations-v28.compose.yaml"),
        )
    )
    failures.extend(
        _runbook_failures(_read(root / "docs/operations/current-head-v28.md"))
    )
    ...
    for marker in (
        ...
        if marker not in sidebar:
            failures.append("current-head-v28-sidebar-open")
            break

    invocation_v27 = "python -B scripts/verify_current_head_v27.py"
    invocation_v28 = "python -B scripts/verify_current_head_v28.py"
    for relative in (
        ".github/workflows/ci.yml",
        ".github/workflows/private-server-runtime-release.yml",
    ):
        source = _read(root / relative)
        if source.count(invocation_v27) != 0 or source.count(invocation_v28) != 1:
            failures.append("current-head-v28-workflow-pointer-open")
            break
    ...
    for relative in (
        ...
        if relative not in readiness:
            failures.append("current-head-v28-source-readiness-open")
            break
    ...
    for marker in (
        ...
        "verify_current_head_v28.py",
        "postgres-operations-v28.compose.yaml",
    ):
        if marker not in operations_summary:
            failures.append("current-head-v28-live-operations-pointer-open")
            break

    if (
        _bytes(root / "deploy/postgres-backup-restore.sh")
        != _bytes(root / "deploy/postgres-backup-restore-v28.sh")
        or _bytes(root / "deploy/postgres-core-facts.sql")
        != _bytes(root / "deploy/postgres-core-facts-v28.sql")
    ):
        failures.append("current-head-v28-unversioned-operations-pointer-open")
