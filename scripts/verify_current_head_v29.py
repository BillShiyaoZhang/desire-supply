#!/usr/bin/env python3
"""Read-only verifier for the v29 PostgreSQL/Matching static release head."""
...
SUCCESS = '{"status":"CURRENT_HEAD_V29_STATIC_VERIFIED"}'
HEADS = "18|47|47|5|5|15|15|23|23|10|10|2|2"
IAM_MANIFEST_SHA256 = "257c438e1d44b385b47505e04f0eca001b41e5121a7f996f3f7e0d8b81d913da"
IAM_COMBINED_SHA256 = "abc9924571cecb3027ec29ee7fdf34596bf8682d8b41c62d033964ec3094400f"
...
TRUST_MANIFEST_SHA256 = "0576a8872e2c9783e345d521f151b3d6f9bd7e1d9ee125ee1ef3810e01a05e47"
TRUST_COMBINED_SHA256 = "96ff2fd0b3e32143b4570fff008948d13fbe5f537a746712878bd2cca77255fa"
MATCHING_MANIFEST_SHA256 = "83547a319fb2d1e5cc88131570fc889ac795b0dd30643e9bca565058226f2cb6"
...
HEAD_SQL_SHA256 = {
    "iam": "f850fbb7dab65b3e4ba95d2e60b77f626924f303b02141fcb8c6eee4e9417d29",
    ...
    "trust": "3ceeeea5a90812f29c56f293f4388f55694f1469a80c40763a6e28b16102c9b5",
    "matching": "54f807a3d210095a79164ad8dc2724686c61decde510e47fc3f7d6f606d45ec6",
...
FROZEN_V28 = {'scripts/verify_current_head_v28.py': '01045dba7f5fcf3cf2b6633f25f8c886f68a8d1f8d5c0e40004f8992714f9f04',
 'docs/operations/current-head-v28.md': '47a8c689e9601b1e88dd492e117692fd93c69dabc4cc35e8a3056d65ab29272d',
 ...
 'tests/deployment/fixtures/current-head-v28/matching-v3-manifest.json': 'b6c4169edcaf4c7cb771fde614ef72c3d90d56b4d2f4d5a0a633f8b634adbf18',
 'deploy/postgres-backup-restore-v28.sh': 'af2fe4105600d439b196b59bb9def07cbe913cc296285bae34f22cfefe8d0f6a',
 'deploy/postgres-core-facts-v28.sql': 'e5c1c7514a1db874fd4aaf6e61d6e486782b1d741ff35c00294c785d624c3d53',
 'deploy/postgres-operations-v28.compose.yaml': '47567174216d101eddada34c78ca983172ec606b583047dc1855f6998537b0d1',
 'tests/deployment/fixtures/current-head-v29/iam-manifest.json': 'faa540929a66eeb7ebfe86ca5e43539ef7dcb10424e792ded14252f27c5850a5',
 'tests/deployment/fixtures/current-head-v29/trust-manifest.json': '3fd3089db8139f4e70551f59f8e803fdf2543847d38d08f82f8a050c2dd921e8',
...
FROZEN_V27 = {
    "scripts/verify_current_head_v27.py": "2b6202deb271802009a542edac4e5e12a994519416bc85394e59e7fe9d370b4f",
    "docs/operations/current-head-v27.md": "39b5f18aaa8c70f96d9ac30643178248d073bd5d973fe5e35d8932bf998b04c1",
    "tests/deployment/fixtures/current-head-v27/schema-pins.json": "94d9d11e40910d1c33802b433d5cbd0ac54f1f048f87235077061811e86fb65f",
    "deploy/postgres-backup-restore-v27.sh": "038792a349e66952d2f1de580cb1c5bb66dbf5d6a92ff505e1d6ef6ed90ae8e5",
...
def _operations_failures(script: str, facts: str, overlay: str) -> tuple[str, ...]:
  ...
  postgres-backup-restore-script:
    file: ./deploy/postgres-backup-restore-v29.sh
  postgres-core-facts-sql:
    file: ./deploy/postgres-core-facts-v29.sql
"""
    if overlay != expected_overlay:
        failures.append("postgres-operations-v29-overlay-open")
    if f"EXPECTED_PINS='{HEADS}'" not in script:
        failures.append("postgres-operations-v29-heads-open")
    if f"EXPECTED_CONTRACTS='{EXPECTED_CONTRACTS}'" not in script:
        failures.append("postgres-operations-v29-contracts-open")
    for marker in (
        ...
        if marker not in script:
            failures.append("postgres-operations-v29-matching-contract-open")
            break
    for table in MATCHING_TABLES:
        qualified = f"matching.{table}"
        if script.count(qualified) != 1 or facts.count(qualified) != 1:
            failures.append("postgres-operations-v29-matching-continuity-open")
            break
    for forbidden in (
        "creator_user_id",
        "reviewer_user_id",
        ...
        if forbidden in facts:
            failures.append("postgres-operations-v29-facts-privacy-open")
            break
    return _unique(failures)


def _runbook_failures(value: str) -> tuple[str, ...]:
    begin = "<!-- BEGIN CURRENT_HEAD_V29_CONTRACT -->"
    end = "<!-- END CURRENT_HEAD_V29_CONTRACT -->"
    failures: list[str] = []
    if (
        value.count(begin) != 1
        ...
        failures.append("current-head-v29-markers-open")
    for marker in (
        HEADS,
        IAM_MANIFEST_SHA256,
        IAM_COMBINED_SHA256,
        ...
        MATCHING_MANIFEST_SHA256,
        TAXONOMY_MANIFEST_SHA256,
        "Matching v1-v10 的 27 张 durable domain tables",
        "matching_continuity_counts",
        "STATIC VERIFIED / NOT PRODUCTION EXECUTED",
        '"production_authorized":false',
        "postgres-operations-v29.compose.yaml",
        "v29-iam47-profile5-demand15-trust23-matching10-taxonomy2-drill01",
    ):
        if marker not in value:
            failures.append("current-head-v29-runbook-contract-open")
            break
    return _unique(failures)
...
def verify_repository(root: Path = REPOSITORY_ROOT) -> tuple[str, ...]:
    failures: list[str] = []
    fixture_path = root / "tests/deployment/fixtures/current-head-v29/schema-pins.json"
    fixture_bytes = _bytes(fixture_path)
    fixture = _json(fixture_path)
    if not isinstance(fixture, dict) or fixture_bytes is None:
        failures.append("current-head-v29-fixture-open")
    else:
        canonical = (
        ...
        if fixture_bytes != canonical:
            failures.append("current-head-v29-fixture-canonical-open")
        if (
            ...
            failures.append("current-head-v29-fixture-claim-open")
        if fixture.get("components") != EXPECTED_COMPONENT_PINS:
            failures.append("current-head-v29-fixture-pins-open")

    if "__MATCHING_V4_" in "\n".join(
        (
            MATCHING_MANIFEST_SHA256,
            _read(fixture_path),
            _read(root / "deploy/postgres-backup-restore-v29.sh"),
            _read(root / "docs/operations/current-head-v29.md"),
        )
    ):
        failures.append("current-head-v29-matching-pins-open")

    failures.extend(_manifest_failures(root))
    failures.extend(_historical_prefix_failures(root))
    for relative, expected in {**FROZEN_V26, **FROZEN_V27, **FROZEN_V28}.items():
    ...
    for name, markers in {
        ...
        "trust": (
            "TRUST_REQUIRED_IAM_SCHEMA_VERSION = 47",
        ...
        "matching": (
            "MATCHING_REQUIRED_IAM_SCHEMA_VERSION = 47",
            "MATCHING_REQUIRED_TRUST_SCHEMA_VERSION = 23",
            TRUST_COMBINED_SHA256,
            TRUST_MANIFEST_SHA256,
        ),
    }.items():
    ...
    failures.extend(
        _operations_failures(
            _read(root / "deploy/postgres-backup-restore-v29.sh"),
            _read(root / "deploy/postgres-core-facts-v29.sql"),
            _read(root / "deploy/postgres-operations-v29.compose.yaml"),
        )
    )
    failures.extend(
        _runbook_failures(_read(root / "docs/operations/current-head-v29.md"))
    )

    sidebar = _read(root / "docs/_sidebar.md")
    for marker in (
        "[Current-head v29 静态模式头](/operations/current-head-v29.md)",
        "[Current-head v27 静态模式头](/operations/current-head-v27.md)",
    ):
        if marker not in sidebar:
            failures.append("current-head-v29-sidebar-open")
            break

    invocation_v27 = "python -B scripts/verify_current_head_v27.py"
    invocation_v29 = "python -B scripts/verify_current_head_v29.py"
    for relative in (
        ".github/workflows/ci.yml",
        ".github/workflows/private-server-runtime-release.yml",
    ):
        source = _read(root / relative)
        if source.count(invocation_v27) != 0 or source.count(invocation_v29) != 1:
            failures.append("current-head-v29-workflow-pointer-open")
            break
    ...
    for relative in (
        ...
        if relative not in readiness:
            failures.append("current-head-v29-source-readiness-open")
            break
    ...
    for marker in (
        "current-head v29",
        "IAM47/Profile5/Demand15/Trust23/Matching10/Taxonomy2",
        "verify_current_head_v29.py",
        "postgres-operations-v29.compose.yaml",
    ):
        if marker not in operations_summary:
            failures.append("current-head-v29-live-operations-pointer-open")
            break

    if (
        _bytes(root / "deploy/postgres-backup-restore.sh")
        != _bytes(root / "deploy/postgres-backup-restore-v29.sh")
        or _bytes(root / "deploy/postgres-core-facts.sql")
        != _bytes(root / "deploy/postgres-core-facts-v29.sql")
    ):
        failures.append("current-head-v29-unversioned-operations-pointer-open")
