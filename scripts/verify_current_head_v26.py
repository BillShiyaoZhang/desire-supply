#!/usr/bin/env python3
"""Read-only verifier for the IAM43 / Demand13 / Trust19 static head."""
...
HEADS = "18|43|43|3|3|13|13|19|19|2|2"
...
IAM_MANIFEST_SHA256 = "7edad01ff151168e4e048848fe770eb0ea199a1034a8119658a1c3bf53205b5e"
IAM_COMBINED_SHA256 = "bb2b025fb26974cf06574117d8e055144d9413c81c035595458c24181f29c72e"
IAM_SQL_SHA256 = "1e6d005858bef6f8dfbcbba2db20f4d515970fa78000b766eed55db4bc4f89df"
PROFILE_MANIFEST_SHA256 = "4f9ac3595a1a6e90cb1f369593bd8fe2483ca7d5ede7dc6d5ae3c2fe469672aa"
DEMAND_API_SHA256 = "046561ae51d147e8df3b8fcf0b61f1dd922efe452175e63f128a937e8f11c4ff"
DEMAND_EVENT_SHA256 = "46631be37cb70aea771d2103e1fe39dc39f3f4303239ae1dc6e55fa946d1059c"
DEMAND_MANIFEST_SHA256 = "5663d8e14bb5fa6a5706828fe443a8c08ac2e62bad3e56403dd45bc6df939b29"
DEMAND_DEPENDENCY_SHA256 = "e3e7a77aeec447cc3035472c5f660c8675238fe260081ce9cedf4dc014b37001"
DEMAND_SQL_SHA256 = "2c4eae15aa6985474042254ce50ef71d15f9efe97bf580f45c1f3a0463857327"
...
TRUST_SQL_SHA256 = "a8dc9b4ba6dbb8a4d1b2e89155745bf30fa617cdbe6fbe6c93a918f277d0c85e"
TRUST_MANIFEST_SHA256 = "5949f7b630376a59c643f9024210625811606a1a41f90f4bc99ee19dfb99d38c"
TRUST_COMBINED_SHA256 = "16913f8503da5e27be72321a3311025bba9a6cf454f8b8b5dad9b4a09ad3417d"
...
API_COMPOSITION_SHA256 = "9215d8243d264c79fc24d4c38f221bd521d7d83d061402bc745b9d82cecf89b6"
API_COMPOSITION_TEST_SHA256 = "6a51e2d29aa11feacbbdc0fb6c4c109dced7bfb9b1fc5beff4d09bd9a83f9f4f"
...
POSTGRES_OPERATIONS_V26_SCRIPT_SHA256 = (
    "48fe07e4a845738cd620b2584eae984d1a66d2258f6fc2c46b0ee63eaec2d72c"
)
POSTGRES_OPERATIONS_V26_FACTS_SHA256 = (
    "274cf10f533673a1541f9dd186039153605bc420f847fa14110027bd5650f153"
)
POSTGRES_OPERATIONS_V26_OVERLAY_SHA256 = (
    "7fc79306fce5feb2d985390ab6e8f6a77955a5ad7d53bb341e25c8ed0df1e041"
)
POSTGRES_OPERATIONS_V26_TEST_SHA256 = (
    "e9243aac88214db2e159e92a793e9a3044cd2b53378951a26a44e2d81e99347e"
)
POSTGRES_OPERATIONS_V25_PINS = "18|42|42|3|3|12|12|18|18|2|2"
POSTGRES_OPERATIONS_V25_CONTRACTS = (
...
EXPECTED_POSTGRES_OPERATIONS_V26_OVERLAY = """configs:
...
"""
POSTGRES_OPERATIONS_V26_RELEASE_EMPTY_TARGET_LINE = (
    "                (SELECT count(*) FROM "
    "demand.demand_review_assignment_releases) +\n"
)
POSTGRES_OPERATIONS_V26_RELEASE_FACT_BLOCK = """        'demand_review_assignment_releases', (
...
SUCCESS = '{"status":"CURRENT_HEAD_V26_STATIC_VERIFIED"}'
...
def _runbook_failures(value: str) -> tuple[str, ...]:
    begin = "<!-- BEGIN CURRENT_HEAD_V26_CONTRACT -->"
    end = "<!-- END CURRENT_HEAD_V26_CONTRACT -->"
    if (
        value.count(begin) != 1
        or value.count(end) != 1
        or value.find(begin) >= value.find(end)
    ):
        return ("current-head-v26-markers-open",)
    required = (
        HEADS,
        EXPECTED_CONTRACTS,
        IAM_SQL_SHA256,
        ...
        "tests/deployment/fixtures/current-head-v26/",
        ...
        "v26-iam43-profile3-demand13-trust19-taxonomy2-drill01",
    ...
    if any(marker not in value for marker in required):
        return ("current-head-v26-contract-open",)
    return ()


def _postgres_operations_v26_failures(
    v25_script: str,
    v26_script: str,
    v25_facts: str,
    v26_facts: str,
    ...
    failures: list[str] = []
    if overlay != EXPECTED_POSTGRES_OPERATIONS_V26_OVERLAY:
        failures.append("postgres-operations-v26-overlay-open")
    if (
        f"EXPECTED_PINS='{POSTGRES_OPERATIONS_V25_PINS}'" not in v25_script
        or f"EXPECTED_CONTRACTS='{POSTGRES_OPERATIONS_V25_CONTRACTS}'"
        not in v25_script
    ):
        failures.append("postgres-operations-v25-history-drifted")
    if (
        f"EXPECTED_PINS='{HEADS}'" not in v26_script
        or f"EXPECTED_CONTRACTS='{EXPECTED_CONTRACTS}'" not in v26_script
    ):
        failures.append("postgres-operations-v26-pins-open")
    normalized = v26_script.replace(HEADS, POSTGRES_OPERATIONS_V25_PINS).replace(
        EXPECTED_CONTRACTS,
        POSTGRES_OPERATIONS_V25_CONTRACTS,
    )
    if normalized.count(POSTGRES_OPERATIONS_V26_RELEASE_EMPTY_TARGET_LINE) != 1:
        failures.append("postgres-operations-v26-release-empty-target-open")
    else:
    ...
    if normalized != v25_script:
        failures.append("postgres-operations-v26-clone-drifted")

    if v26_facts.count(POSTGRES_OPERATIONS_V26_RELEASE_FACT_BLOCK) != 1:
        failures.append("postgres-operations-v26-release-facts-open")
    else:
        if (
            v26_facts.replace(
                POSTGRES_OPERATIONS_V26_RELEASE_FACT_BLOCK,
                "",
                1,
            )
            != v25_facts
        ):
            failures.append("postgres-operations-v26-facts-clone-drifted")
...
def verify_repository(root: Path = REPOSITORY_ROOT) -> tuple[str, ...]:
    ...
    v25_root = root / "tests/deployment/fixtures/current-head-v25"
    assets = (
        ...
        (
            iam_root / "0043_expand__demand_review_assignment_release_authority.sql",
            IAM_SQL_SHA256,
        ),
        (profile_root / "manifest.json", PROFILE_MANIFEST_SHA256),
        (demand_root / "manifest.json", DEMAND_MANIFEST_SHA256),
        (demand_root / "0013_expand__review_assignment_release.sql", DEMAND_SQL_SHA256),
        (trust_root / "manifest.json", TRUST_MANIFEST_SHA256),
        (
            trust_root / "0019_expand__iam43_demand13_dependency_repin.sql",
            TRUST_SQL_SHA256,
        ),
        ...
        (root / "platform/contracts/api/demand-v1.openapi.yaml", DEMAND_API_SHA256),
        (
            root / "platform/contracts/events/demand-v1.schema.json",
            DEMAND_EVENT_SHA256,
        ),
        ...
        (
            root / "deploy/postgres-core-facts-v26.sql",
            POSTGRES_OPERATIONS_V26_FACTS_SHA256,
        ),
        (
        ...
        (
            root / "tests/deployment/test_postgres_operations_v26.py",
            POSTGRES_OPERATIONS_V26_TEST_SHA256,
        ),
    )
    for path, expected in assets:
        if _sha(path) != expected:
            failures.append(
                f"current-head-v26-asset-mismatch:{path.relative_to(root)}"
            )

    if _manifest_versions(fixture_root / "iam-manifest.json", "iam") != tuple(
        range(44)
    ):
        failures.append("current-head-v26-iam-sequence-open")
    if _manifest_versions(
        fixture_root / "demand-manifest.json", "demand"
    ) != tuple(range(1, 14)):
        failures.append("current-head-v26-demand-sequence-open")
    if _manifest_versions(
        fixture_root / "trust-manifest.json", "trust"
    ) != tuple(range(1, 20)):
        failures.append("current-head-v26-trust-sequence-open")
    if _iam_combined(root) != IAM_COMBINED_SHA256:
        failures.append("current-head-v26-iam-combined-open")

    failures.extend(
        _postgres_operations_v26_failures(
            _read(root / "deploy/postgres-backup-restore-v25.sh"),
            _read(root / "deploy/postgres-backup-restore-v26.sh"),
            _read(root / "deploy/postgres-core-facts-v25.sql"),
            _read(root / "deploy/postgres-core-facts-v26.sql"),
            _read(root / "deploy/postgres-operations-v26.compose.yaml"),
        )
    )
    ...
    for source, markers, failure in (
        (
            iam_catalog,
            ("demand_review_assignment_release_authority", IAM_MANIFEST_SHA256),
            "current-head-v26-iam-catalog-open",
        ),
        (
            _read(demand_root / "catalog.py"),
            ("review_assignment_release", DEMAND_MANIFEST_SHA256),
            "current-head-v26-demand-catalog-open",
        ),
        (
            trust_catalog,
            ("iam43_demand13_dependency_repin", TRUST_MANIFEST_SHA256),
            "current-head-v26-trust-catalog-open",
        ),
        (
            trust_runner,
            (
                "TRUST_REQUIRED_IAM_SCHEMA_VERSION = 43",
                "TRUST_REQUIRED_DEMAND_SCHEMA_VERSION = 13",
                IAM_COMBINED_SHA256,
                DEMAND_DEPENDENCY_SHA256,
                TRUST_API_SHA256,
                TRUST_APPEAL_API_SHA256,
            ...
            "current-head-v26-trust-runner-open",
        ),
        (
            frozen_runner,
            (
            ...
            "current-head-v26-frozen-runner-open",
        ),
    ):
    ...
    demand13_sql = _read(
        demand_root / "0013_expand__review_assignment_release.sql"
    )
    trust19_sql = _read(
        trust_root / "0019_expand__iam43_demand13_dependency_repin.sql"
    )
    release_service = _read(
        root
        / "platform/src/desire_platform/internal_pilot/editor/postgres_service.py"
    )
    release_repository = _read(
        root / "platform/src/desire_platform/internal_pilot/editor/postgres.py"
    )
    release_http = _read(
        root / "platform/src/desire_platform/internal_pilot/editor/http.py"
    )
    release_uow = _read(
        root / "platform/src/desire_platform/demand/adapters/postgres/uow.py"
    )
    release_static_tests = "\n".join(
        _read(root / path)
        for path in (
            "platform/tests/storage/postgres/"
            "test_iam43_demand_review_assignment_release_authority_static.py",
            "platform/tests/storage/postgres/"
            "test_demand13_review_assignment_release_static.py",
        )
    )
    release_runtime_tests = "\n".join(
        _read(root / path)
        for path in (
            "platform/tests/internal_pilot/"
            "test_demand_review_assignment_release.py",
            "platform/tests/storage/postgres/test_demand_postgres_red.py",
            "platform/tests/storage/postgres/"
            "test_demand_review_queue_postgres.py",
        )
    )
    demand_api = _read(root / "platform/contracts/api/demand-v1.openapi.yaml")
    demand_events = _read(root / "platform/contracts/events/demand-v1.schema.json")
    demand_contract_tests = _read(
        root / "platform/tests/contract/test_demand_contracts.py"
    )
    web_release = "\n".join(
        _read(root / path)
        for path in (
            "web/lib/app-contract.mjs",
            "web/lib/app-contract.d.mts",
            "web/lib/server-proxy.mjs",
            "web/app/product-client.tsx",
            "web/tests/app-contract.test.mjs",
            "web/tests/local-contract.test.mjs",
            "web/tests/product-shell.test.mjs",
        )
    )
    for source, markers in (
        ...
        (
            release_service + release_repository + release_http + release_uow,
            (
                "release_demand_review_assignment",
                "read_completed_release",
                "RELEASE_REVIEW_ASSIGNMENT",
                "ReleaseDemandReviewAssignment",
                "DemandReviewAssignmentReleased",
                "CONFLICT_DECLARED",
                "WORKLOAD_RELEASE",
                "review_assignment_release.insert",
                "/v1/app/demands/",
                "/review-assignments/",
                "/release",
            ),
        ...
        (
            release_static_tests + release_runtime_tests,
            (
                "test_iam43_adds_exactly_one_operation_to_both_v2_closed_sets",
                "test_release_fact_is_closed_immutable_force_rls_and_role_minimal",
                "test_completed_receipt_replay_is_exact_authority_bound_and_read_only",
                "test_conflict_release_excludes_all_same_reviewer_reclaim_paths_only",
                "test_release_builds_exact_assignment_bound_idempotent_command",
                "test_completed_release_replay_precedes_active_assignment_discovery",
                "test_review_assignment_release_persists_one_exact_atomic_fact_graph",
                "review_claim_conflict_declared",
            ),
        ),
        (
            demand_api + demand_events + demand_contract_tests,
            (
                ...
                "WORKLOAD_RELEASE",
                "DemandReviewAssignmentReleased",
                "test_review_assignment_release_contract_is_closed_and_returns_demand_update",
                "test_review_assignment_release_event_has_exact_public_payload",
            ),
        ),
        (
            web_release,
            (
                ...
                "REVIEW_RELEASE_ROUTE",
                "validateReviewReleaseProxyResponse",
                "释放当前审核分配（非最终决定）",
                "重新回到审核队列",
                "Operations review release BFF validates the closed request",
            ),
        ),
    ):
        if any(marker not in source for marker in markers):
            failures.append("current-head-v26-review-release-boundary-open")
            break
    for sql in (iam43_sql, demand13_sql):
        if "DROP TABLE" in sql.upper() or "TRUNCATE TABLE" in sql.upper():
            failures.append("current-head-v26-forward-only-open")
            break
    ...
    for source, markers in (
        ...
        if any(marker not in source for marker in markers):
            failures.append("current-head-v26-public-name-boundary-open")
            break
    ...
    for source, markers in (
        ...
        (
            ...
            (
                "test_demand12_remains_the_byte_exact_historical_prefix",
        ...
        if any(marker not in source for marker in markers):
            failures.append("current-head-v26-finance-history-boundary-open")
            break
    ...
    for source, markers in (
        ...
        if any(marker not in source for marker in markers):
            failures.append("current-head-v26-trust-history-boundary-open")
            break
    ...
    for source, markers in (
        ...
        if any(marker not in source for marker in markers):
            failures.append("current-head-v26-appeal-history-boundary-open")
            break
    ...
    for source, markers in (
        ...
        if any(marker not in source for marker in markers):
            failures.append("current-head-v26-http-observability-open")
            break
    ...
    for forbidden in (
        ...
        if forbidden in boundary_projection:
            failures.append("current-head-v26-http-observation-privacy-open")
            break
    ...
    for source in (compose, compose_dev, operations_compose, real_oidc_compose):
        if any(marker not in source for marker in logging_values):
            failures.append("current-head-v26-bounded-compose-logging-open")
            break
    for source, markers in (
        (
        ...
        if any(marker not in source for marker in markers):
            failures.append("current-head-v26-bounded-docker-logging-open")
            break

    failures.extend(
        _runbook_failures(_read(root / "docs/operations/current-head-v26.md"))
    )
    ...
    for marker in (
        ...
        if marker not in sidebar:
            failures.append("current-head-v26-sidebar-open")
            break

    v25_verifier = _read(root / "scripts/verify_current_head_v25.py")
    for marker in (
        "tests/deployment/fixtures/current-head-v25",
        "18|42|42|3|3|12|12|18|18|2|2",
        "919d62a9b32dc29619a561940f9c79e21700562a0cdd9ef7e9f13a40cea43345",
        ...
        if marker not in v25_verifier:
            failures.append("current-head-v25-frozen-pins-open")
            break
    ...
    v26_invocation = "python -B scripts/verify_current_head_v26.py"
    if (
        ci.count(v25_invocation) != 0
        or ci.count(v26_invocation) != 1
        or release.count(v25_invocation) != 0
        or release.count(v26_invocation) != 1
    ):
        failures.append("current-head-v26-workflow-pointer-open")
    ...
    for forbidden in (
        ...
        if forbidden in operations_summary:
            failures.append("current-head-v26-operations-stale-pointer-open")
            break
    for marker in (
        "IAM `0043`",
        "Demand `0013`",
        "Trust `0019`",
        "IAM43/Demand13/Trust19 current-head v26",
    ):
        if marker not in operations_summary:
            failures.append("current-head-v26-operations-summary-open")
            break
    return _unique(failures)
