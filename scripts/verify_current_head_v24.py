...
SUCCESS = '{"status":"CURRENT_HEAD_V24_STATIC_VERIFIED"}'
...
def _runbook_failures(value: str) -> tuple[str, ...]:
    begin = "<!-- BEGIN CURRENT_HEAD_V24_CONTRACT -->"
    end = "<!-- END CURRENT_HEAD_V24_CONTRACT -->"
    if (
        value.count(begin) != 1
        or value.count(end) != 1
        or value.find(begin) >= value.find(end)
    ):
        return ("current-head-v24-markers-open",)
    required = (
        HEADS,
        EXPECTED_CONTRACTS,
        IAM_SQL_SHA256,
    ...
    if any(marker not in value for marker in required):
        return ("current-head-v24-contract-open",)
    return ()
...
def verify_repository(root: Path = REPOSITORY_ROOT) -> tuple[str, ...]:
    ...
    fixture_root = root / "tests/deployment/fixtures/current-head-v24"
    v23_root = root / "tests/deployment/fixtures/current-head-v23"
    assets = (
    ...
    for path, expected in assets:
        if _sha(path) != expected:
            failures.append(
                f"current-head-v24-asset-mismatch:{path.relative_to(root)}"
            )

    if _manifest_versions(fixture_root / "iam-manifest.json", "iam") != tuple(
        range(43)
    ):
        failures.append("current-head-v24-iam-sequence-open")
    if _manifest_versions(
        fixture_root / "demand-manifest.json", "demand"
    ) != tuple(range(1, 13)):
        failures.append("current-head-v24-demand-sequence-open")
    if _manifest_versions(
        fixture_root / "trust-manifest.json", "trust"
    ) != tuple(range(1, 19)):
        failures.append("current-head-v24-trust-sequence-open")
    if _iam_combined(root) != IAM_COMBINED_SHA256:
        failures.append("current-head-v24-iam-combined-open")
    ...
    for source, markers, failure in (
        ...
        (
            ...
            "current-head-v24-trust-runner-open",
        ),
        (
            frozen_runner,
            (
            ...
            "current-head-v24-frozen-runner-open",
        ),
    ):
    ...
    for source, markers in (
        ...
        if any(marker not in source for marker in markers):
            failures.append("current-head-v24-public-name-boundary-open")
            break
    for forbidden in ("DROP TABLE", "TRUNCATE TABLE"):
        if forbidden in public_name_sql.upper():
            failures.append("current-head-v24-forward-only-open")
            break
    ...
    for source, markers in (
        ...
        if any(marker not in source for marker in markers):
            failures.append("current-head-v24-finance-history-boundary-open")
            break
    ...
    for source, markers in (
        ...
        if any(marker not in source for marker in markers):
            failures.append("current-head-v24-trust-history-boundary-open")
            break
    ...
    for source, markers in (
        ...
        if any(marker not in source for marker in markers):
            failures.append("current-head-v24-appeal-history-boundary-open")
            break

    failures.extend(
        _runbook_failures(_read(root / "docs/operations/current-head-v24.md"))
    )
    ...
    for marker in (
        ...
        if marker not in sidebar:
            failures.append("current-head-v24-sidebar-open")
            break

    v23_verifier = _read(root / "scripts/verify_current_head_v23.py")
    for marker in (
        "tests/deployment/fixtures/current-head-v23",
        "18|42|42|3|3|12|12|17|17|2|2",
        "919d62a9b32dc29619a561940f9c79e21700562a0cdd9ef7e9f13a40cea43345",
        ...
        if marker not in v23_verifier:
            failures.append("current-head-v23-frozen-pins-open")
            break
    ...
    for forbidden in (
        ...
        if forbidden in operations_summary:
            failures.append("current-head-v24-operations-stale-pointer-open")
            break
    for marker in (
        "IAM `0042`",
        "Demand `0012`",
        "Trust `0018`",
        "IAM42/Demand12/Trust18 current-head v24",
    ):
        if marker not in operations_summary:
            failures.append("current-head-v24-operations-summary-open")
            break
