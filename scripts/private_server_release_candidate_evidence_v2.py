#!/usr/bin/env python3
"""Generate or verify fail-closed current-head v14 caller claims.

Version 2 cannot verify evidence provenance and therefore always remains
...
SCHEMA_VERSION = "desire-private-server-release-candidate-evidence-v2"
BLOCKED = "BLOCKED"
...
_CANDIDATE_ID = re.compile(
    r"^current-head-v14-rc-[a-z0-9][a-z0-9._-]{0,63}$"
)
...
BLOCKING_REASONS = (
    ...
    "TRUST_APPLICANT_DISCOVERY_DEFERRAL_NOT_ACCEPTED",
    "ONE_SHOT_V14_NOT_UNCONSUMED_VERIFIED",
)
...
def _validate_trust_applicant_discovery_deferral(
    value: Any,
    snapshot_sha256: str | None,
) -> None:
    deferral = _require_object(
        value,
        (
    ...
    _require_const(deferral["implementation_status"], "DEFERRED_NOT_IMPLEMENTED")
    _require_const(deferral["boundary"], "TRUST_APPLICANT_DISCOVERY_DEFERRED")
    accepted = deferral["accepted_for_candidate_scope"]
    if type(accepted) is not bool:
        _invalid()
    record = _require_sha256(deferral["acceptance_record_sha256"], nullable=True)
    bound_snapshot = _require_sha256(
        deferral["source_snapshot_sha256"], nullable=True
    )
    if not accepted:
...
def _derived_blocking_reasons(document: Mapping[str, Any]) -> tuple[str, ...]:
    # v2 validates caller-supplied shape and local bindings only.  It does not
    ...
    if not document["trust_applicant_discovery_deferral"][
        "accepted_for_candidate_scope"
    ]:
        reasons.append(BLOCKING_REASONS[6])
    if document["one_shot_v14"]["claim"] != "UNCONSUMED_VERIFIED":
...
def validate_release_candidate_evidence(document: Any) -> str:
    ...
    _require_const(candidate["candidate_scope"], "RUN_CURRENT_HEAD_V14_ONCE")
    ...
    _validate_trust_applicant_discovery_deferral(
        candidate["trust_applicant_discovery_deferral"],
        snapshot_sha256,
    )
    _validate_one_shot(candidate["one_shot_v14"], snapshot_sha256)
