#!/usr/bin/env python3
"""Read-only verifier for the IAM42 / Demand12 / Trust17 static head."""
...
HEADS = "18|42|42|3|3|12|12|17|17|2|2"
...
TRUST_SQL_SHA256 = "9ec66244773c7546537bb41a7c93c518f804947ddb88d8f14eb5e32e191b0854"
TRUST_MANIFEST_SHA256 = "57c0dd42e18bf3afa7233f9ad673ec3805b325166436a4a1e3021466cd62381f"
TRUST_COMBINED_SHA256 = "a1ec68f0d0e6685e0cbe842a6bd951f60f334682d26bec549ef9858c81f23d67"
...
SUCCESS = '{"status":"CURRENT_HEAD_V23_STATIC_VERIFIED"}'
...
def _runbook_failures(value: str) -> tuple[str, ...]:
    begin = "<!-- BEGIN CURRENT_HEAD_V23_CONTRACT -->"
    end = "<!-- END CURRENT_HEAD_V23_CONTRACT -->"
    if (
        value.count(begin) != 1
        or value.count(end) != 1
        or value.find(begin) >= value.find(end)
    ):
        return ("current-head-v23-markers-open",)
    required = (
        HEADS,
        EXPECTED_CONTRACTS,
        IAM_SQL_SHA256,
        ...
        "tests/deployment/fixtures/current-head-v23/",
    ...
    if any(marker not in value for marker in required):
        return ("current-head-v23-contract-open",)
    return ()
...
def verify_repository(root: Path = REPOSITORY_ROOT) -> tuple[str, ...]:
    ...
    fixture_root = root / "tests/deployment/fixtures/current-head-v23"
    v22_root = root / "tests/deployment/fixtures/current-head-v22"
    assets = (
        ...
        (trust_root / "0017_expand__completed_case_history_http_contract.sql", TRUST_SQL_SHA256),
    ...
    for path, expected in assets:
        if _sha(path) != expected:
            failures.append(
                f"current-head-v23-asset-mismatch:{path.relative_to(root)}"
            )

    if _manifest_versions(fixture_root / "iam-manifest.json", "iam") != tuple(
        range(43)
    ):
        failures.append("current-head-v23-iam-sequence-open")
    if _manifest_versions(
        fixture_root / "demand-manifest.json", "demand"
    ) != tuple(range(1, 13)):
        failures.append("current-head-v23-demand-sequence-open")
    if _manifest_versions(
        fixture_root / "trust-manifest.json", "trust"
    ) != tuple(range(1, 18)):
        failures.append("current-head-v23-trust-sequence-open")
    if _iam_combined(root) != IAM_COMBINED_SHA256:
        failures.append("current-head-v23-iam-combined-open")
    ...
    for source, markers, failure in (
        (
            ...
            "current-head-v23-iam-catalog-open",
        ),
        (
            _read(demand_root / "catalog.py"),
            ("finance_funding_terminal_history", DEMAND_MANIFEST_SHA256),
            "current-head-v23-demand-catalog-open",
        ),
        (
            trust_catalog,
            ("completed_case_history_http_contract", TRUST_MANIFEST_SHA256),
            "current-head-v23-trust-catalog-open",
        ),
        (
            trust_runner,
            (
            ...
            "current-head-v23-trust-runner-open",
        ),
        (
            frozen_runner,
            (
            ...
            "current-head-v23-frozen-runner-open",
        ),
    ):
    ...
    for source, markers in (
        ...
        if any(marker not in source for marker in markers):
            failures.append("current-head-v23-public-name-boundary-open")
            break
    for forbidden in ("DROP TABLE", "TRUNCATE TABLE"):
        if forbidden in public_name_sql.upper():
            failures.append("current-head-v23-forward-only-open")
            break
    ...
    for source, markers in (
        ...
        if any(marker not in source for marker in markers):
            failures.append("current-head-v23-finance-history-boundary-open")
            break
    ...
    for source, markers in (
        ...
        if any(marker not in source for marker in markers):
            failures.append("current-head-v23-trust-history-boundary-open")
            break

    failures.extend(
        _runbook_failures(_read(root / "docs/operations/current-head-v23.md"))
    )
    ...
    for marker in (
        ...
        if marker not in sidebar:
            failures.append("current-head-v23-sidebar-open")
            break

    v22_verifier = _read(root / "scripts/verify_current_head_v22.py")
    for marker in (
        "tests/deployment/fixtures/current-head-v22",
        "18|42|42|3|3|12|12|16|16|2|2",
        "919d62a9b32dc29619a561940f9c79e21700562a0cdd9ef7e9f13a40cea43345",
        ...
        if marker not in v22_verifier:
            failures.append("current-head-v22-frozen-pins-open")
            break
    ...
    for forbidden in (
        ...
        if forbidden in operations_summary:
            failures.append("current-head-v23-operations-stale-pointer-open")
            break
    for marker in (
        "IAM `0042`",
        "Demand `0012`",
        "Trust `0017`",
        "IAM42/Demand12/Trust17 current-head v23",
    ):
        if marker not in operations_summary:
            failures.append("current-head-v23-operations-summary-open")
            break
