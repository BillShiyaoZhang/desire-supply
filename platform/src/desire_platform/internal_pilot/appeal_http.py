"""Authenticated ASGI boundary for the closed Appeal HTTP presenter."""
...
class AppealAsgiApplication:
    """Authenticate, select one exact workspace, and dispatch one Appeal call."""
    ...
    def __init__(
        ...
        *,
        ...
        if not isinstance(dispatcher, AppealHttpApplicationDispatcher):
            raise TypeError("Appeal HTTP dispatcher is unavailable")
        if not callable(getattr(session_security, "authenticate", None)) or not callable(
            getattr(session_security, "require_valid", None)
        ):
            raise TypeError("Appeal Session security is unavailable")
        if not callable(getattr(principal_resolver, "resolve", None)):
            raise TypeError("Appeal workspace resolver is unavailable")
        if not allowed_origins or len(set(allowed_origins)) != len(allowed_origins):
            raise TypeError("Appeal origin allowlist is unavailable")
        for origin in allowed_origins:
            ...
            if not (valid_https or valid_internal):
                raise TypeError("Appeal origin allowlist is invalid")
        if (
            ...
            raise TypeError("Appeal transport configuration is invalid")
    ...
    async def _handle(
        ...
        *,
        ...
        in_namespace = isinstance(path, str) and (
            path == "/v1/app/appeals"
        ...
        if isinstance(query, AppealHttpResponse):
            return query

        headers = _headers(scope.get("headers", ()))
        if isinstance(headers, AppealHttpResponse):
            return headers
        ...
        applicant = path == "/v1/app/appeals" or path.startswith(
            "/v1/app/appeals/"
        )
        if not _principal_matches(principal, applicant=applicant):
            return _error(404, "RESOURCE_NOT_FOUND")
        try:
            actor = _actor_context(authenticated, principal, applicant=applicant)
        ...
        if not isinstance(response, AppealHttpResponse):
            return _error(503, "SERVICE_UNAVAILABLE")
        return response
...
def _query(
    *, path: str, method: str, raw: bytes
) -> Mapping[str, Tuple[str, ...]] | AppealHttpResponse:
    collection_read = method == "GET" and path == "/v1/app/appeals"
    if not raw:
        return (
            _error(400, "INVALID_REQUEST", "/query")
            if collection_read
            else {}
        )
    ...
    try:
        ...
        return _error(400, "INVALID_REQUEST", "/query")
    prefix = "source_outcome_version_id="
    ...
    if (
        ...
        or "%" in text
        or "+" in text
        or _UUID_TEXT.fullmatch(value) is None
    ):
        return _error(400, "INVALID_REQUEST", "/query")
    return {"source_outcome_version_id": (value,)}


def _principal_matches(principal: EditorPrincipal, *, applicant: bool) -> bool:
    if applicant:
        return (
            ...
            and principal.organization_role_codes
            == tuple(sorted(set(principal.organization_role_codes)))
            and "DEMAND_OWNER" in principal.role_codes
            and "DEMAND_OWNER" in principal.organization_role_codes
        )
    return (
        ...
        and principal.role_codes == tuple(sorted(set(principal.role_codes)))
        and principal.platform_duty_codes
        == tuple(sorted(set(principal.platform_duty_codes)))
        and "APPEAL_REVIEWER" in principal.role_codes
        and "APPEAL_REVIEWER" in principal.platform_duty_codes
    )
...
def _actor_context(
    ...
    *,
    ...
    if (
        ...
        raise ValueError("Appeal actor mismatch")
    return TrustActorContext(
        actor_user_id=user_id,
        session_id=session_id,
        organization_id=(
            _canonical_uuid(principal.organization_id) if applicant else None
        ),
...
def _headers(value: Any) -> Mapping[str, Tuple[str, ...]] | AppealHttpResponse:
...
def _error(status: int, code: str, path: str | None = None) -> AppealHttpResponse:
...
async def _send(send: AsgiSend, response: AppealHttpResponse) -> None:
    try:
        body = json.dumps(
            response.json,
            allow_nan=False,
