-- Metadata-only Trust direct dependency repin from reviewed IAM0039 to
-- IAM0040. Demand0011 and its published transitive IAM0037 ABI stay frozen.
...
BEGIN
    SELECT
        ...
        COALESCE(
            bool_and(
                ...
                AND schema_head_version = 12
                AND min_app_compatible_version = 12
                AND max_app_compatible_version = 12
                AND required_iam_schema_version = 39
    ...
    THEN
        RAISE EXCEPTION USING
            ERRCODE = '55000',
            MESSAGE = 'TRUST12_SCHEMA_CONTRACT_BASELINE_MISMATCH';
    END IF;
END
$trust12_contract_baseline$;
...
ADD CONSTRAINT ck_trust_schema_contract_versions CHECK (
    ...
    AND min_app_compatible_version = 13
    AND max_app_compatible_version = 13
    AND required_iam_schema_version = 40
    AND required_demand_schema_version = 11
),
