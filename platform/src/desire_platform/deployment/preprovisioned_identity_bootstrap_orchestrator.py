"""Closed handoff for deployment-time preprovisioned OIDC identities.

The generator and the existing bootstrap parser, apply program, and verifier
run in one process.  Only the generated manifest digest crosses phases; raw
provider subjects and email values never enter this orchestration surface.
"""
...
class PreprovisionedIdentityBootstrapOrchestrationError(RuntimeError):
    """Stable, non-reflective orchestration failure."""
...
class PreprovisionedIdentityBootstrapOrchestrationReport:
    manifest_sha256: str
    apply_outcome: str
    verify_outcome: str
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ValueError(
                "preprovisioned identity bootstrap report is invalid"
            )


def _invalid() -> NoReturn:
    raise PreprovisionedIdentityBootstrapOrchestrationError(
...
def _failed() -> NoReturn:
    raise PreprovisionedIdentityBootstrapOrchestrationError(
        "PREPROVISIONED_IDENTITY_BOOTSTRAP_ORCHESTRATION_FAILED"
    )


def orchestrate_preprovisioned_identity_bootstrap(
    *,
    environment: Mapping[str, str],
    clock: Callable[[], datetime] = lambda: datetime.now(timezone.utc),
    generate_manifest: Callable[
        ..., Any
    ] = generate_preprovisioned_identity_bootstrap_manifest_file,
    ...
    dbapi: Any = psycopg,
) -> PreprovisionedIdentityBootstrapOrchestrationReport:
    """Generate, parse, apply, and verify one exact reviewed manifest."""
    ...
    try:
        ...
        generated = generate_manifest(
            environment={key: values[key] for key in _GENERATOR_ENVIRONMENT},
            now=now,
        )
        ...
        if (
            ...
            _failed()
        return PreprovisionedIdentityBootstrapOrchestrationReport(
            manifest_sha256=digest,
            apply_outcome=apply_outcome,
            verify_outcome=verify_outcome,
        )
    except PreprovisionedIdentityBootstrapOrchestrationError:
        raise
    except (
        PreprovisionedIdentityBootstrapManifestGenerationError,
        IdentityBootstrapError,
...
def main(
    ...
    *,
    ...
    try:
    ...
    except PreprovisionedIdentityBootstrapOrchestrationError as error:
        stderr.write(
            json.dumps(
        ...
        stderr.write(
            '{"code":"PREPROVISIONED_IDENTITY_BOOTSTRAP_ORCHESTRATION_FAILED",'
            '"status":"BLOCKED"}\n'
        )
        return 78
    stdout.write(
        json.dumps(
            {
                ...
                "status": (
                    "PREPROVISIONED_IDENTITY_BOOTSTRAP_ORCHESTRATION_READY"
                ),
