...
MIGRATION = MIGRATION_ROOT / "0013_expand__iam40_dependency_repin.sql"
...
TRUST13_SQL_SHA256 = (
    "d5d714824f8b20d1bbbcaed2bd0ee7e8f38ef42c5aa749189c7fa6bf407bf00e"
)
TRUST13_MANIFEST_SHA256 = (
    "c438a3fac4d9dea850089b8a14f92ab34a5c5a592b9babcb770860d3ecc513d8"
)
TRUST13_COMBINED_SHA256 = (
...
def test_trust13_remains_the_frozen_historical_prefix() -> None:
    catalog = TrustMigrationCatalog.load(MIGRATION_ROOT)
    descriptor = catalog.artifacts[12].descriptor

    assert TRUST_MIGRATION_LAYOUT[12] == (
        13,
        TrustMigrationPhase.EXPAND,
        "iam40_dependency_repin",
        MIGRATION.name,
    )
    assert descriptor.checksum_sha256.hex() == TRUST13_SQL_SHA256
    assert hashlib.sha256(MIGRATION.read_bytes()).digest() == (
        descriptor.checksum_sha256
    )
    assert catalog.artifacts[11].descriptor.prefix_manifest_sha256.hex() == (
        FROZEN_TRUST12_MANIFEST_SHA256
    )
    assert descriptor.prefix_manifest_sha256.hex() == TRUST13_MANIFEST_SHA256


def test_trust13_pins_exact_iam40_and_preserves_demand11_dependency() -> None:
...
def test_trust13_sql_requires_exact_trust12_baseline_then_repins_metadata() -> None:
    sql = MIGRATION.read_text(encoding="utf-8")
    compact = " ".join(sql.split())

    for marker in (
        "TRUST12_SCHEMA_CONTRACT_BASELINE_MISMATCH",
