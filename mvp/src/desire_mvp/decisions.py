...
def validate_decision(
    ...
    if not isinstance(reason_code, str) or reason_code not in codes:
        raise DecisionError("未知决定原因代码")
    ...
    if any(not isinstance(creator_id, str) for creator_id in invited_creator_ids):
        raise DecisionError("被邀请者 ID 必须是字符串")
    unknown_invited = set(invited_creator_ids) - candidates
    if unknown_invited:
        raise DecisionError("被邀请者不在合格候选中")
    ...
    for response in participant_responses:
        ...
        if not isinstance(creator_id, str):
            raise DecisionError("候选反馈 creator_id 必须是字符串")
        if creator_id not in invited_creator_ids:
            raise DecisionError("候选反馈来自未邀请者")
        if creator_id in seen:
            raise DecisionError("候选反馈重复")
        if not isinstance(code, str) or code not in response_codes:
            raise DecisionError("未知候选反馈代码")
