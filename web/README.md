...
- 按“工作、协作、管理、个人”组织的职责导航；每次聚焦一个工作区，首页默认显示待处理任务；
- 画像和需求按目的分步填写、检查后保存或提交；版本与技术信息按需展开；
- 独立的组织成员、邀请、组织信息视图；账号安全与业务操作分开；
...
完整的信息架构、各角色旅程、交互和视觉规范见 [系统 UI/UX 设计](../docs/design/ui-ux-system.md)。模块切换保留当前组件的输入与恢复状态；未确认写入会显露其所属页面并锁住导航。编辑器离开前继续使用原有当前标签页草稿恢复机制。
