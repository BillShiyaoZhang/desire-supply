...
<!-- BEGIN CURRENT_HEAD_V25_CONTRACT -->
...
当前字节 pin 冻结在 `tests/deployment/fixtures/current-head-v25/`。其中 IAM、Demand、Trust manifest
与 Trust runner pin 都是只读静态 fixture，不是运行结果。

v25 不前移任何数据库版本；它把以下 privacy-safe HTTP observability 与 bounded local Docker
logging 源码和关闭测试作为同一静态发布的精确 byte pins：
...
| API composition / composition test | `629b3f43490386baa9c328a3c002ad1910dfc34186b41febee4e33926e00e25f` / `5de0d58517d245145a5b9ec52d27856a588d3eee854653b7457efb24881c31f1` |
...
| v25 PostgreSQL backup/restore script / durable facts | `9aa84d3f7d37704e181a314db873e16fecfad6d770dbc1d12fbb76180d69d1bb` / `0845ec9025efdfc208bab24b1ce3b8f56a8e2e44613eae249a00af349802507e` |
| v25 PostgreSQL operations overlay / static test | `a98b80de17604349362b813d1224a4f71d886b2d1282f9cbb944cd3b714628a4` / `5296e02cf37a5ffdf54603639202e6f074138706832d811355ded15efe3da383` |
...
与 `trust_terminal_history_discoverable=true`、`terminal_history_actor_scoped=true` 验证，v25 不扩大该读取面。
...
当前头新增 `deploy/postgres-backup-restore-v25.sh`、
`deploy/postgres-core-facts-v25.sql` 与 `deploy/postgres-operations-v25.compose.yaml`。脚本把
backup/restore 的模式门禁固定到 IAM42 / Profile3 / Demand12 / Trust18 / Taxonomy2 及本页十段契约；
overlay 只重绑脚本和 facts 两个 config，不复制或改变 service、image、command、secret、volume 或 network。
旧 v13/v14/v15 脚本、facts 与 overlay 保持历史字节不变。

v25 的 `iam_durable_counts` 在共享去标识摘要之外，增加 organization、external identity、contact point、
...
compose_v25_operations() {
  docker compose \
    ...
    -f "$PWD/deploy/postgres-operations-v25.compose.yaml" \
    "$@"
}

export DESIRE_DATABASE_BACKUP_BASENAME="v25-iam42-profile3-demand12-trust18-taxonomy2-drill01"
```
...
两条成功日志。因此状态继续是 `STATIC VERIFIED / NOT EXECUTED`，不是 backup/restore 动态证据。
...
python3 -B -m unittest tests.deployment.test_current_head_v25_contract -v
python3 -B -m unittest tests.deployment.test_postgres_operations_v25 -v
```

v25 校验成功只输出 `{"status":"CURRENT_HEAD_V25_STATIC_VERIFIED"}`。校验只读取已签入文件，不调用
Docker，不运行迁移，不创建 evidence，也不把 `NOT EXECUTED` 升级成动态成功。

## v24 冻结边界

[Current-head v24 静态模式头](/operations/current-head-v24.md) 继续保持
`18|42|42|3|3|12|12|18|18|2|2`。其 fixture 位于
`tests/deployment/fixtures/current-head-v24/`；v25 的 HTTP observation 与 bounded Docker logging
声明不得写回或冒充 v24 结果。v24 verifier、runbook、IAM/Demand/Trust fixtures 与 runner pin 分别冻结为
...
所有已经单独记录的 v24 dynamic evidence 都是冻结历史，不能重标为 v25 动态执行；v25 仍是
`STATIC VERIFIED / NOT EXECUTED`，不构成生产授权。

```json
{"current_head_v25":{"claim":"STATIC_ONLY","execution":"NOT_EXECUTED"},"overall_status":"BLOCKED","production_authorized":false}
```

<!-- END CURRENT_HEAD_V25_CONTRACT -->

与本静态合同分开记账的 checkout runtime/source 本地合成验收见
[本地 INTERNAL_SANDBOX 试用管理器](/operations/local-internal-sandbox-trial.md#当前-v25-checkout-本地合成动态验收2026-08-26)。
该记录不改变本页的 `STATIC VERIFIED / NOT EXECUTED`、`overall_status=BLOCKED` 或
`production_authorized=false`。
