...
| Trust 0016 SQL | `46bd5355dffb1028d11f277b785cc8e03266b49c3d9e5dbe0a5a954b0ecdb08d` |
| Trust manifest / combined | `71b61f666ea9d924a7edae14db1bf3cc20905618d806d0c8e76b94066c07672c` / `d1df1117a20361e041a2da24b79a8408c05f4ea949c8a930e3cb3634d2f6a04e` |
| Trust required IAM / Demand schema | `42 / 12` |
| Taxonomy manifest | `74153871acb734ad3fe7619174695af57a751b64fd0aba2165ed776ec2538622` |

当前字节 pin 冻结在 `tests/deployment/fixtures/current-head-v22/`。其中 IAM、Demand、Trust manifest
...
Trust16 是 metadata-only dependency repin：业务 Trust 表与 API 不扩权，只把已审查依赖从 Demand11
推进到 Demand12，并重新绑定 Demand dependency、Trust manifest 与 combined contract。
...
python3 -B -m unittest tests.deployment.test_current_head_v22_contract -v
```

v22 校验成功只输出 `{"status":"CURRENT_HEAD_V22_STATIC_VERIFIED"}`。校验只读取已签入文件，不调用
Docker，不运行迁移，不创建 evidence，也不把 `NOT EXECUTED` 升级成动态成功。

## v21 冻结边界

[Current-head v21 静态模式头](/operations/current-head-v21.md) 继续保持
`18|42|42|3|3|11|11|15|15|2|2`。其 fixture 位于
`tests/deployment/fixtures/current-head-v21/`；v22 的 Demand12、Trust16、财务本人完成历史或升级 preflight
声明不得写回或冒充 v21 结果。v21 的 `STATIC VERIFIED / NOT EXECUTED` 声明也没有被提升为动态执行或生产授权。

```json
{"current_head_v22":{"claim":"STATIC_ONLY","execution":"NOT_EXECUTED"},"overall_status":"BLOCKED","production_authorized":false}
```

<!-- END CURRENT_HEAD_V22_CONTRACT -->
