...
> `0003`、Taxonomy `0002`，对应
> [Current-head v27 静态模式头](/operations/current-head-v27.md)。v27 把独立 Matching runtime、
> selector/reviewer workbench、固定 Demand delivery/completion 与零候选关闭纳入当前运行和恢复合同。
> 该版本仅为 `STATIC VERIFIED / NOT PRODUCTION EXECUTED`，没有 v27 fresh-volume、production
> migration、backup/restore 或生产部署动态证据。冻结 v26/v25 发布资产以及绑定当时 checkout
...
`deploy/postgres-operations-v27.compose.yaml` 签入并由同一 verifier 检查，但本次没有调用 Docker
或创建 artifact/恢复资源。
