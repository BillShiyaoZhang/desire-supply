...
Matching9 / Taxonomy2。workflow 在构建前运行 current-head v29 静态 verifier，对
IAM47/Profile5/Demand15/Trust23/Matching10/Taxonomy2 做独立只读检查；新 runtime release 的同一模式头
绑定还由本版本 core、schema 与 bundle 合同闭合。v29 前向修复 Matching ingest 名称歧义和 coordinator
...
一致。随后 workflow 还会运行 current-head v29 的只读静态 verifier；它闭合当前
