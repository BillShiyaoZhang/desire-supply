...
当前新生成制品的模式头固定为 PostgreSQL18 / IAM48 / Profile5 / Demand16 / Trust24 /
Matching11 / Taxonomy2。workflow 在构建前运行 current-head v30 静态 verifier，对
IAM48/Profile5/Demand16/Trust24/Matching11/Taxonomy2 做独立只读检查；新 runtime release 的同一模式头
绑定还由本版本 core、schema 与 bundle 合同闭合。v30 加入管理员需求全流程只读查询，不改变
...
一致。随后 workflow 还会运行 current-head v30 的只读静态 verifier；它闭合当前
