...
> 授权边界：本文不自动授权生产开发。`DEC-033` 仅替代 `G1-02` 的真人研究前置；G1 其余条件不变。`G1` 前仅允许工程事实核验、已知风险修复、合成数据验证和抛弃式原型；`G1` 通过后才允许按本文范围实现最窄生产纵切。真实用户、真实产品数据、合同或资金仍须通过 `G2`。
...
`DEC-033` 允许团队在上述范围内直接关闭 G1 的非研究条件，但不能把 Slice 实现、Demo 或测试结果写成市场/需求证据；所有相关假设保持 `E0`。
