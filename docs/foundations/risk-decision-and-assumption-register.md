...
| `RSK-002` | 把理念认同、访谈意向或样例当付费需求 | `HIGH/UNKNOWN` | 证据等级与研究协议 | 真实行动/采购/合同/付款分别记录，反例纳入决定 | Research / `G1/G2` | `BLOCKER` |
...
假设证据按[研究与证据计划](/foundations/research-and-evidence-plan.md)升级；任何假设不得因团队已经写代码而提高置信度。
...
- `G1` 存在多个 `BLOCKER`，因此生产纵切开发为 `NO-GO — pending G1`；
