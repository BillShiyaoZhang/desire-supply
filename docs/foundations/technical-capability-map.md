...
Capability 是责任和验收边界，不等于部署单元；多个 Capability 可以先由同一模块承载。只有在数据所有权、团队责任、扩缩容或故障隔离确有需要时，才考虑拆成独立服务。
...
| --- | --- | --- |
| `FND-HUM-*` | `CAP-C01/02/04/09`, `CAP-I03/11` | 意愿与边界表达、拒绝无报复、多重角色、关系与质性结果 |
| `FND-COL-*` | `CAP-C02..09`, `CAP-S03/07` | 真实需求、双向接受、协议/变更、可核验付款、争议与再次合作 |
...
| `CAP-S06` | Web BFF & Accessible Product Shell | 浏览器会话、CSRF、输入契约、权限化导航、无障碍、国际化、错误恢复、运营/成员不同视图、端到端测试 | 前端不得重新实现授权或隐藏失败；重要决定不能只用通知表达 | 尚无实际产品壳 |
...
| `CAP-C02` | Demand & Public-Value Intake | 决策权、受益者、问题、范围、验收、预算、付款计划、风险、AI/数据边界、商业或公共价值轨道、审核与版本 | 没有真实决策权、资金路径或可验收范围不得进入匹配；支付能力不等于公共价值 | MVP 可运行；平台 Domain/PG 局部存在，后续状态链未闭合 |
...
| `CAP-C05` | Project, Agreement & Delivery | 项目建立、协议版本与双方接受、里程碑、输入请求、交付、验收、拒收、范围变更、终止、完成 | 修改范围同时重新确认时间/报酬；验收人与需求决策权清晰；历史协议不可静默改写 | 仅有设计 |
...
| `CAP-C09` | Outcome, Review & Relationship | 已对账付款/时效、范围变化、争议、安全事件、双方评价、需求清晰度、再次合作意愿、公共影响、运营成本 | 财务事实与参与者自报观察分开；评价有事实范围、回应/纠错权和时间背景；不生成全局人格分 | MVP 有自报 Outcome/Report；平台 Audit/Analytics/Review 尚为设计 |
...
flowchart LR
    ...
    J --> A["AgreementVersion accepted by both"]
...
| `CAP-I08` | Capital & Dependency Transparency | 投资者/客户/资助/采购来源投影、权利类别、债务/附带条件、收入集中度、事实否决风险、阈值预警和公开摘要 | 不在产品库复制完整法定 cap table；敏感财务与公开透明需分层 | 当前仅 foundations 原则和少量运营指标设想 |
...
组织完成身份和政策接受，创建有决策权与资金路径的 Demand；创作者按目的披露 Profile；Demand 在匹配前获得 `DEMAND_VERSION` Funding 保障；系统执行硬边界和可解释匹配；双方独立确认后创建 Project shell 并接受 AgreementVersion；Milestone 在需要时获得独立 Funding 后开工；交付、验收、付款、对账、结果和评价形成完整证据链。
...
9. 把每个设计页的当前状态改为能力登记册引用，而不是独立维护的自然语言结论。
