...
- Platform 对 Python `3.9` 与 `3.14` 建立矩阵，每个版本都使用 digest-pinned PostgreSQL 18.4 跑分发测试与完整 suite；2026-09-04 修复 Ubuntu 24.04 的安装兼容性后，精确版本为 `3.9.25` 与 `3.14.1`（原 `3.9.6` 无对应 runner 构建）。每个版本的完整 unittest suite 按模块稳定分为四片，保留类和模块 fixture 边界，覆盖所有发现的测试，避免单个 job 的 30 分钟时限截断后续测试；
